...
- Ajuste do layour do FlatList pra modo de 2 colunas (grid)
...
- Adição e configuração de fonte personalizada usando o `expo-font` e `use-fonts`
...
\_Utilizamos o `expo install` para garantir a instalação de dependências (ou libs) que sejam compatíveis com a versão de SDK do Expo usada em seu projeto.
